...
import { ApiOptionsInterface, ApiObservationsSeilflyItem } from 'app/api/api.interface';
...
@Component({
  selector: 'nlf-ors-seilfly-report',
  templateUrl: './ors-seilfly-report.component.html',
  styleUrls: ['./ors-seilfly-report.component.css'],
})
export class NlfOrsSeilflyReportComponent implements OnInit {

  @Input() inputId?: number; // Allows to use seperately

  id: number;
  version: number;
  observation: ApiObservationsSeilflyItem;
  dataDiff: any;
  dataReady = false;
  error;
  spinner = true;
