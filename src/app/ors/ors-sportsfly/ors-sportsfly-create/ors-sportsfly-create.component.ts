...
@Component({
  selector: 'nlf-ors-sportsfly-create',
  templateUrl: './ors-sportsfly-create.component.html',
  styleUrls: ['./ors-sportsfly-create.component.css']
})
export class NlfOrsSportsflyCreateComponent implements OnInit {
  ...
  @Input() activity: string = 'sportsfly';
  userid: number;
  ...
  public canCreate() {
    try {
      ...
      return this.config.sportsfly.observation.create[this.ENV._name];
    } catch (e) {
      console.log(e);
      return false;
    }
            ...
            this.orsService.create({ discipline: this.selected, club: element.parent_id, occurrence: occurrence}).subscribe(
              data => {
                ...
                if (!!data._id && !!data.id) {
                  ...
                  this.router.navigateByUrl('/ors/sportsfly/edit/' + data.id);
                }
              },
              err => {
